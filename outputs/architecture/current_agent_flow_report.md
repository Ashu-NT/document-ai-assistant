...
7. ~~`IngestionRequest.source_name` is accepted but not persisted by the current document model.~~ — **stale claim, corrected 2026-07-04**. Checked directly: `IngestionWorkflow.run` sets `parsing_result.document_graph.document.source_name = request.source_name` (§2.2), `DocumentORM.source_name` is a real column, and the domain entity carries it too — it's persisted correctly. No code change was needed, this was a documentation error in the original review.
...
1. There are two active answer surfaces, `ask_document.py` and LangGraph, so UX and safeguards can drift. **Partially addressed 2026-07-04**: the concrete, actionable half of this — "`ask_document.py` has no live progress visibility at all" — is fixed. `QuestionAnsweringWorkflow.run()` gained an additive `progress_callback: Callable[[str], None] | None = None` parameter (same established pattern as `IngestionWorkflow`/`ParsingWorkflow`), emitting stage messages (analyzing, retrieving, checking guardrails, generating, done) at each major phase; threaded through `AnswerQuestionRequest.progress_callback` → `AnswerQuestionTool.run()` → the workflow. `ask_document.py` now passes `print_status` as the callback (suppressed under `--json`). This is deliberately the lightweight `progress_callback`-string pattern, not the heavier LangGraph-specific `EventStreamAdapter`/`LiveAgentEvent`/`ConsoleLiveEventSink` machinery the demo path uses — that machinery is built around iterating a compiled LangGraph's per-node `.stream()` output and doesn't apply to `QuestionAnsweringWorkflow`'s straight-line execution. The broader "polish" half of this item (unifying final-answer *presentation*, not just progress visibility, across `ask_document.py`/`agent_cli.py`/`demo_agent_cli.py`) remains open — see §9 P3#1.
...
6. Reflection is limited to one retry by default; that is safe, but shallow for hard cases. It's also now carrying substantial hand-tuned, question-shape-specific logic (maintenance-interval, spare-parts, identifier-inventory) inside the validator, which is effective but adds maintenance burden — every new "shape" of question risks needing its own downgrade rule. **Investigated but deliberately not refactored, 2026-07-04**: `ReflectionValidator` (`src/application/langgraph/reflection/validation/reflection_validator.py`) is 515 lines, a single ~300-line `validate()` method with 15-18 distinct branches over decision types (`RETRIEVE_AGAIN`/`CLARIFY`/`FAIL`) combined with hand-computed boolean context flags and keyword-list constants — genuinely the "one big function with many special cases" shape this item describes, not yet modularized into discrete rule objects. This item is explicitly framed as "a maintenance-burden risk, not a bug" — restructuring it carries real regression risk on a sensitive path (it governs whether user-facing answers get accepted, retried, or failed) without changing any actual behavior, which is a different risk/reward trade-off than the other items in this cycle (all of which fixed a concrete, verifiable defect). Left for a dedicated follow-up with the user's explicit sign-off on taking that risk, rather than restructuring sensitive logic unprompted mid-cycle.
...
9. ~~Internal agent/runtime composition still lives heavily in scripts and runtime factories rather than one universal application entry surface.~~ — **the concrete duplication closed 2026-07-04; the "one universal entry surface" framing itself was too broad and is not being pursued as originally framed.** Investigated what was actually duplicated: the SQL+dense hybrid retrieval stack (vector store, keyword index, reranker, `RetrievalWorkflow`) was re-derived near-verbatim in three places — `build_agent_runtime` (`demo_agent_runtime.py`), `build_qa_runtime` (`ask_document.py`), `build_benchmark_runtime` (`run_retrieval_benchmark.py`) — each with its own private `_create_qdrant_client` copy. Extracted a new `src/application/orchestrator/retrieval/build_retrieval_runtime()`, mirroring the ingestion side's existing `build_ingestion_runtime()` pattern (and reusing its `build_vector_store()` directly rather than re-deriving Qdrant client/vector-store construction a fourth time). Pre/post-retrieval guardrails are passed straight through with no built-in default, since the benchmark deliberately runs without them (to score raw retrieval quality) while the interactive/CLI callers want them — a real behavioral difference the shared function had to preserve, not paper over. All three call sites now delegate to it; each dropped ~15-20 lines of duplicated wiring plus its private `_create_qdrant_client` helper. The ingestion side (`build_ingestion_runtime`/`build_parsing_runtime`/`build_parsing_ocr_runtime`) was already properly layered and needed no change. Merging agent-runtime/ingestion/benchmark/eval into one single universal entry point (the literal "one universal application entry surface" framing) was investigated and rejected: those serve genuinely different lifecycles (interactive session vs. batch corpus scoring vs. offline eval), and forcing them through one entry point would be premature abstraction with no duplication left to justify it.
   - Where: `src/application/orchestrator/retrieval/` (new), `src/application/agent_runtime/demo_agent_runtime.py`, `scripts/ask_document.py`, `scripts/run_retrieval_benchmark.py`
   - Verified: `tests/unit/application/orchestrator/retrieval/test_retrieval_runtime_builder.py` (new, 4 tests: wiring, embedding-provider reuse vs. fresh-build, guardrail pass-through, guardrail defaulting to empty); full `tests/unit` + `tests/integration` unaffected elsewhere.
...
11. ~~No end-to-end test exercises a `planned_task` route through `DocumentAgentGraph` with a real repaired/validated plan, nor a test combining `PlanRepair` + `PlanValidator` against an adversarial malformed LLM plan.~~ — **first half fixed 2026-07-04**. The existing `test_document_agent_graph_executes_validated_llm_plan` fixture plan was already valid on the first `PlanValidator.validate()` pass, so `CreatePlanNode._attempt_llm_plan`'s repair branch (`self.plan_repair.repair(...)`) was never actually exercised end-to-end — only `PlanRepair`/`PlanValidator` in isolation had unit coverage. Added `test_document_agent_graph_executes_repaired_llm_plan`: a fixture plan using the nonexistent tool name `"ask_question"` (which fails initial validation — not allowed by policy, not in the tool registry, all its args count as unsupported), gets renamed to `"answer_question"` by `PlanRepair`, re-validates successfully, and executes for real through the graph. Asserts `planning_source == "repaired"` and the rename shows up in `planning_warnings`. The adversarial-malformed-plan variant (still open) would need a plan `PlanRepair` itself can't fix (e.g. a required step using a blocked tool) to exercise the full-failure path at the graph level — not attempted here.
   - Where: `tests/unit/application/langgraph/graphs/test_document_agent_graph.py`
12. ~~No CLI-level test verifies `scripts/demo_agent_cli.py` actually wires `EventStreamAdapter`/`ConsoleLiveEventSink` end-to-end during a real interactive run.~~ — **fixed 2026-07-04**. The wiring actually lives in `DemoAgent.execute_graph_command` (`src/application/agent_runtime/demo_agent.py`), not `demo_agent_cli.py` itself. Added 3 tests: a fake runtime that plays back real `LiveAgentEvent`s through whatever `event_sink` it's given (mirroring what `EventStreamAdapter` does while iterating the compiled graph's node stream) confirms `execute_graph_command` (a) constructs a genuine `ConsoleLiveEventSink` bound to real `sys.stdout` (captured via `capsys`, not a mock) when not quiet/JSON, and that emitted events actually appear in real captured output; (b) falls back to `NullEventSink` — and produces zero output — in `--quiet` mode; (c) same for `--json` mode.
   - Where: `tests/unit/application/agent_runtime/test_demo_agent.py`
...
| Planning (deterministic + LLM repair/validation) | `tests/unit/application/langgraph/planning/test_deterministic_planner.py`, `test_llm_plan_proposer.py`, `test_plan_parser.py`, `test_plan_policy.py`, `test_plan_validator.py`, `test_plan_repair.py`, `test_plan_prompt_builder.py`, `test_plan_executor.py`, `test_plan_models.py`, `tests/unit/application/langgraph/graphs/test_document_agent_graph.py::test_document_agent_graph_executes_repaired_llm_plan` (added 2026-07-04) | ~~no integration/end-to-end test running a `planned_task` route through `DocumentAgentGraph` with a real repaired/validated plan~~ fixed 2026-07-04; still no test combining `PlanRepair` + `PlanValidator` in sequence against an adversarial malformed LLM plan that `PlanRepair` itself cannot fix |
...
| Agent-runtime live streaming | `tests/unit/application/agent_runtime/streaming/test_live_agent_event.py`, `test_live_event_sink.py`, `test_console_event_sink.py`, `test_event_stream_adapter.py`, `test_agent_loop_style.py`, `tests/unit/application/agent_runtime/test_live_react_streaming.py`, `tests/unit/cli_scripts/test_demo_agent_cli_live_output.py`, `tests/unit/application/agent_runtime/test_demo_agent.py` (+3 tests, added 2026-07-04) | ~~no CLI-level test verifying `scripts/demo_agent_cli.py` actually wires `EventStreamAdapter`/`ConsoleLiveEventSink` end-to-end during a genuinely interactive run~~ fixed 2026-07-04 — see §3.15 item 12 |
...
1. Unify final-answer presentation quality across `ask_document.py`, `agent_cli.py`, and `demo_agent_cli.py`.
   ...
   - **Partially done 2026-07-04**: the live-streaming half is fixed — `ask_document.py` now gets real progress visibility via a new `progress_callback` on `QuestionAnsweringWorkflow.run()` (see §3.15 item 1). The broader answer-*presentation* unification (formatting, citation display, structure) is still open — this was deliberately scoped down since presentation-quality unification is a larger, more subjective UX task than closing a concrete "no progress output at all" gap.
...
3. ~~Add a CLI-level end-to-end test for the live-streaming demo path.~~ — **fixed 2026-07-04**; duplicate of §3.15 item 12 (same gap, listed twice in the original review). The wiring actually lives in `DemoAgent.execute_graph_command`, not `demo_agent_cli.py` directly. Added 3 tests in `tests/unit/application/agent_runtime/test_demo_agent.py` — see §3.15 item 12 for details.
