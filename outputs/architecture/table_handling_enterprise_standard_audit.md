...
### F12 — Minor / style (not blocking) — Status: item 2 resolved by decision, items 1 and 3 implemented

- **Item 1 — implemented.** `_INTERVAL_HEADER_PATTERN` hardcoded D/W/M/Q/S/A schedule vocabulary directly in the generic row-grid builder. Extracted to a new isolated, swappable class `CompactIntervalHeaderTokenMatcher` (`src/application/workflows/parsing/tables/structure/compact_interval_header_token_matcher.py`), injected into `DoclingTableRowGridBuilder` via constructor (`interval_header_matcher` param, same DI pattern as `row_repairer`), matching the audit's own "no document-specific value rules in the generic parser" principle. Behavior verified byte-identical via the existing `test_build_rows_distributes_compact_interval_headers_across_columns` test (unchanged, still passing) plus a new dedicated `test_compact_interval_header_token_matcher.py` (6 tests) for the extracted class in isolation.
- **Item 2 — decided, no code change.** Asked directly: should the structure-detection layer (`TableSemanticResolver`'s shape/quality/header-path computation) get a feature flag, and default on or off? Decision: no flag. The feature is no longer "new/unshipped" — real documents in the DB already carry `table_shape`/`table_structure_quality`/`header_paths` from it, and F10 wired those fields all the way through chunking into the DB. Gating it now would mean actively disabling something already relied upon, not shipping a new feature safely. Left as-is.
- **Item 3 — implemented.** `table_semantic_rule_evaluator.py` was 354 lines, over the repo's ~300-LOC split convention. Split by rule family (no facade — `TableSemanticClassifier` now depends on both split classes directly, not via delegation):
  - `TableSemanticRuleEvaluator` (130 lines) keeps `looks_like_maintenance_interval_table`/`looks_like_lubrication_schedule_table`/`looks_like_troubleshooting_table`.
  - New `TableSpecificationRuleEvaluator` (210 lines) takes `looks_like_operation_reference_table`/`looks_like_operating_limits_table`/`looks_like_technical_data_table`/`looks_like_certification_table`.
  - New `TableBodyTextExtractor` (35 lines) takes the unrelated `body_label_cells`/`body_text` helpers (row/text extraction, not classification rules — a distinct responsibility that didn't belong bundled into the rule evaluator regardless of file length).
  - `TableSemanticClassifier` now constructs and calls `self.rule_evaluator`, `self.specification_rule_evaluator`, and `self.body_text_extractor` directly. Verified against the full existing `test_table_semantic_classifier.py` suite (16 tests, all still passing unchanged) — confirms the split is behavior-preserving.

Full `tests/unit` suite after all three F12 items: 2739 passed, 0 failed.
...
14. **F12** — minor/style items. **Done** — item 2 (feature flag) resolved by an explicit product decision (no flag: the feature is already relied upon, not a new/unshipped one); items 1 (`CompactIntervalHeaderTokenMatcher` extraction) and 3 (`table_semantic_rule_evaluator.py` split into `TableSemanticRuleEvaluator`/`TableSpecificationRuleEvaluator`/`TableBodyTextExtractor`, no facade) implemented and verified behavior-preserving against existing tests.

Every fix shipped with a regression test reproducing the exact failure mode found (or, for F10, exercising the exact propagation path; for F11/F12, exercising the previously-untested or refactored real behavior directly), verified against a full green `tests/unit` run before moving to the next item. Final state: every finding in this audit (F1-F13, F9b, F12) is fixed, implemented, or explicitly resolved by decision. Full `tests/unit` suite: 2739 passed, 0 failed.
