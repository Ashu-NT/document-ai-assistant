...
Three findings are data-integrity issues with direct, demonstrated real-world impact:
...
### F9b — Docling-derived `column_count` metadata can itself under-report the real row width, independent of column-dropping (NEW, OPEN — found via the same corpus scan)

Found while verifying F9 against the real corpus: 9 of the 592 scanned tables have `declared_column_count < actual_row_width` (e.g. `declared_column_count=1` while `table.rows` entries have 2-3 cells), all on simple list/TOC-style tables (`Number | Title | Page`, `P&ID Pos Nr. | Service Function Type | Part No.`). This is the opposite direction from F9 and cannot be produced by `drop_globally_empty_columns` (which only ever removes columns, never adds row width beyond the original). The `max_span_col_end` for these tables is internally consistent with the (wrong) `declared_column_count`, so the root cause sits further upstream — most likely in how `table_rows`/`column_count` are independently derived from Docling's raw structure for this table shape (worth checking `docling_table_row_grid_builder.py` and `docling_table_extractor.py`'s `extract_dimensions`, both touched by F7/F8, for a case where the row-splitting path produces more cells per row than the reported grid dimensions).

**Status**: not investigated further or fixed in this pass — flagged with the real document/table IDs (all in `01 Operating Manual High Pressure Compressors MV320 20251125` and `19P006-31-FWC12-5-1-0_Manual`) for a follow-on pass.
...
11. **F9b** — newly discovered during F9's corpus verification: `column_count` metadata itself under-reporting actual row width, unrelated to column-dropping. **Not investigated further** — logged with real document/table IDs for a follow-on pass.
...
Every fix shipped with a regression test reproducing the exact failure mode found (or, for F10, exercising the exact propagation path; for F11, exercising the previously-untested real behavior directly), verified against a full green `tests/unit` run before moving to the next item. Final state: F1-F11 and F13 fixed/implemented and verified (F9 additionally verified at scale against the real DB corpus); F9b newly discovered and documented for a follow-on pass. Full `tests/unit` suite: 2728 passed, 0 failed.
