...
EQUIPMENT_EXTRACTION_PROMPT_VERSION = "v1"


class EquipmentExtractionPromptBuilder:
    prompt_version = EQUIPMENT_EXTRACTION_PROMPT_VERSION
    metadata = PromptMetadata(
        name="equipment_extraction",
        version=EQUIPMENT_EXTRACTION_PROMPT_VERSION,
        task_type="extraction",
        model_type="llm",
        description="Extract named equipment/machinery with model, serial, and manufacturer name from chunks.",
    )
    ...
    def build(
        ...
        *,
        ...
        return (
            ...
            f"{EQUIPMENT_SCHEMA_TEXT}"
            "}\n"
            f"{EQUIPMENT_GUIDANCE}"
            f"{EQUIPMENT_EXTRACTION_EXAMPLE}"
