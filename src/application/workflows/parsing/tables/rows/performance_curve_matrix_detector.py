...
from src.application.workflows.parsing.tables.rows.table_row_patterns import normalize_cell
...
class PerformanceCurveMatrixDetector:
    ...
    def _has_curve_block(
        ...
        *,
        ...
        if consecutive_numeric_columns < 3 or inspected_cells == 0:
            return False
...
def _looks_textual_header(value: str) -> bool:
    cleaned = normalize_cell(value)
    if not cleaned or _looks_numericish(cleaned):
        return False
