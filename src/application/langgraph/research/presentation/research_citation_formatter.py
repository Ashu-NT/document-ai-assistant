...
class ResearchCitationFormatter:
    def display_document_name(self, value: str | None) -> str:
        if isinstance(value, str):
            ...
            if normalized and not re.fullmatch(r"doc[_-][a-f0-9]{8,}", normalized, re.IGNORECASE):
                return normalized
    ...
    def format_page_list(self, spans: list[tuple[int, int]]) -> str | None:
        if not spans:
            return None
        ordered = self._merge_page_spans(sorted(set(spans), key=lambda item: (item[0], item[1])))
        labels = [
    ...
    def build_reference_entries(self, references: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for reference in references:
            ...
            if simplified_path and simplified_path not in entry["sections"]:
                entry["sections"].append(simplified_path)
