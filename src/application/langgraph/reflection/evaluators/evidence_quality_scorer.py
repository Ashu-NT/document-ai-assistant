...
class EvidenceQualityScorer:
    ...
    def score(
        *,
        ...
        referenced_pages: list[int] | None = None,
    ) -> EvidenceQuality:
        ...
        normalized_referenced_pages = sorted(set(referenced_pages or []))
        missing_pages = (
            sorted(set(normalized_pages) - set(normalized_referenced_pages))
            if referenced_pages is not None
            else []
        )
        page_coverage_ratio = 1.0
        if referenced_pages is not None and normalized_pages:
            page_coverage_ratio = len(
                set(normalized_pages).intersection(normalized_referenced_pages)
            ) / len(normalized_pages)
            if missing_pages:
                issues.append("partial_page_coverage")

        if reference_notes is None and referenced_pages is None:
            ...
            resolved_notes = sum(
                1 for note in notes if _reference_note_chunk_id(note) is not None
            )
            ...
            score = round(
                (
                    (1.0 if has_sufficient_evidence else 0.0)
                    + (0.0 if has_document_leakage else 1.0)
                    + (1.0 if normalized_pages else 0.0)
                    + citation_resolution_rate
                    + page_coverage_ratio
                )
                / 5.0,
