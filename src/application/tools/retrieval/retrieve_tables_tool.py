...
class RetrieveTablesRequest(ToolRequest):
...
class RetrieveTablesTool:
    metadata = ToolMetadata(
        tool_name="retrieve_tables",
        category="retrieval",
        description="Retrieve table-oriented chunks or list document tables deterministically.",
        mutates_state=False,
        supports_trace=True,
    )
    ...
    def run(self, request: RetrieveTablesRequest) -> ToolResult:
        ...
        return ToolResult.ok(data=result.tables, metadata=self.metadata)
