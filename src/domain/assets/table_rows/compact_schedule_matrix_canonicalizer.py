...
class CompactScheduleMatrixCanonicalizer:
    """Normalizes compact maintenance schedule matrices into stable rows.

    This handles cases where Docling preserves compact headers such as
    ``D | Q Q | M S A | Task Reference`` but body rows collapse sparse empty
    interval columns, causing task text to drift into schedule columns.
    """
