...
def test_overview_section_bonus_prefers_what_it_does_section() -> None:
    doc = _make_document(title="FWC12 System Manual", file_name="fwc12_manual.pdf")

    chunk_overview = _make_chunk(
        chunk_id="c_overview",
        content=(
    ...
    chunk_install = _make_chunk(
        chunk_id="c_install",
        content=(
            "The FWC system pipe connections are to be in accordance with the mounting "
            "and pipe connection drawing supplied."
        ),
        chunk_type=ChunkType.INSTALLATION_INSTRUCTION,
        section_path='["4 Installation"]',
    )

    query_text = "What does the FWC system do?"
    result_overview = _score(chunk_overview, doc, query_text)
    result_install = _score(chunk_install, doc, query_text)

    assert result_overview.total_score > result_install.total_score
