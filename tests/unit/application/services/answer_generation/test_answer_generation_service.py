...
def test_generate_surfaces_sections_and_reference_notes_from_llm_response() -> None:
    """Plan section 9.6 (sections/reference_notes redesign): optional
    structured breakdown must reach GeneratedAnswer, resolved against the
    real sources used for this generation."""
    service, _ = make_service(
        FakeLLMService(
            response=(
                '{"answer_text":"The filter is replaced every 1000 hours.",'
...
def test_generate_resolves_reference_note_source_number_to_chunk_id() -> None:
    ...
    request = AnswerGenerationRequest(
        question="Question?",
        context_chunks=[
            _make_chunk(chunk_id="chunk_001"),
            _make_chunk(chunk_id="chunk_002"),
        ],
    )
