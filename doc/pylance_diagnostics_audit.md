...
- **Phase B — done.** Fixed all 5 confirmed individual bugs from the Priority 6 table: `scripts/run_retrieval_quality_gate.py` (missing `Path` import — script was completely unrunnable, verified fixed by actually invoking it with `--help`), `scripts/demo_agent_cli.py` (added a `TYPE_CHECKING` import for the deferred `DemoVisibilityPolicy` forward reference), `src/shared/execution/events/event_tracker.py` (annotation corrected to `EventService | None`), `src/application/agent_runtime/session/conversation_turn.py` (dropped the invalid quoted-union return annotation, redundant anyway given `from __future__ import annotations`), and `src/application/workflows/parsing/normalizers/docling_document_normalizer.py` (extracted the sort key into a named `_reading_order_sort_key` static method with an explicit `int(...)` coercion, confirmed `layout_page_order` is always a plain `int` at the point it's produced in `layout_metadata_serializer.py`). Verified: **370 → 364 errors**. Full test suite: same pass count, same single pre-existing unrelated failure.
- **Phase D — done.** Widened Priority 4's LangGraph helpers (`response_text_resolver.py`'s 4 functions, `node_utils.py`'s `resolve_selected_document`, `research_state_mapper.py`'s `result_from_state`) from `dict[str, Any]` to `AgentState` — confirmed every one only reads via `.get()`, no mutation, so this is a pure type-widening with no behavior change. Also widened Priority 5's ORM mapper functions (`ExtractionResultMapper.to_domain`'s 11 row-list params, `document_graph_reader.py`'s `_group_element_ids_by_section`, `ConversationMemoryMapper.to_domain`) from `list[X]` to `Sequence[X]`, and fixed `scripts/report_text_corruption_candidates.py`'s `scan_chunk_rows` to accept `Sequence[Row[tuple[...]]]` (SQLAlchemy's actual `.all()` return shape, not a plain tuple). The `AgentState` widening surfaced one real latent fragility as a side effect: `research_state_mapper.py` was checking `isinstance(state.get("research_result"), dict)` and then separately re-calling `state.get("research_result", {})` in the branch body — two independent lookups the type checker (correctly) can't prove agree, and which in practice only stayed safe because the second call's `{}` default never actually applies once the key exists with value `None`. Fixed by binding the lookup to a local variable once. Verified: **364 → 338 errors**. Full test suite both before and after that follow-up fix: same pass count, same single pre-existing unrelated failure.
- **Phase C — done.** Decided the `confidence_score` nullable-comparison question (`document_classification_validator.py`, `extraction_result_validator.py`) by scanning the codebase for existing convention rather than guessing: `ClassificationResult.confidence_score` and `ExtractionResult.confidence_score` are both `float | None` by deliberate domain design — `ClassificationResult.is_confident()` already treats `None` as "not confident" (a valid state, not an error), both ORM columns (`ClassificationResultORM`, `ExtractionResultORM`) declare `nullable=True`, and on the extraction side `None` is a live, reachable value (the single-response `ExtractionResponsePayload.confidence_score` field is genuinely optional with no fallback in `extraction_result_assembler.py`, unlike the multi-response merger which defaults to `0.0`). Conclusion: `None` means "confidence not determined," a legitimate state — tightening the domain type to non-Optional `float` would be wrong. Fixed both validators to skip the range check when `confidence_score is None` instead of tightening the type. Added regression tests (`test_document_classification_validator_accepts_none_confidence`, `test_extraction_result_validator_accepts_none_confidence`) asserting `confidence_score=None` no longer flags `*.confidence.invalid`. Verified: **338 → 334 errors** (both `reportOptionalOperand` sites plus downstream). Full test suite: 3637 passed, same single pre-existing unrelated failure.
